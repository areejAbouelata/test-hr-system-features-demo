...
const uiLabels = computed(() => {
	return {
		formTitle: isUpdating.value
			? trans("update-job-title")
			: trans("create-job-title"),
...
const store = () => {
	form.post(route("job-title.store"), submitOptions);
};
const update = () => {
	form.patch(route("job-title.update", [props.model]), submitOptions);
};
const submit = () => {
	if (isUpdating.value) {
		update();
	...
	>
		...
		<div class="">
			<Form :id="formId" @submit="submit">
				<BranchesField :form />
				<TranslatableField :label="$t('name')" :form form-key="title">
					<Input />
				</TranslatableField>
