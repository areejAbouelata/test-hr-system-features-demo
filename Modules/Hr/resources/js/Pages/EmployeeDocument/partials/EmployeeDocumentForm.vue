...
const uiLabels = computed(() => {
	return {
		formTitle: isUpdating.value
			...
			: trans("add-document"),
		triggerLabel: isUpdating.value ? trans("update") : trans("add"),
		submitLabel: isUpdating.value ? trans("update") : trans("add"),
	};
});
...
const store = () => {
	form.post(route("employee-document.store"), submitOptions);
};
const update = () => {
	form.patch(route("employee-document.update", [props.model]), submitOptions);
};
const submit = () => {
	if (isUpdating.value) {
		update();
